...
def make_pot_RK(Z):
    ...
    return FuncAtom2D_RytKel(z=-Z)


def make_pot_Gauss(Z):
    from d2_py.FuncAtom2DLn import FuncAtom2DLn
    # potFunc = FuncAtom2DLn(z=-cfg.atom_z, r0=cfg.r0)  # // f(r)=-2*Z*Ln(r)
    return FuncAtom2DLn(z=-Z, r0=cfg.r0)   # use your actual RK class


pots = [ # tag, L, pot
    # ("3D",  0.0,  make_pot_3d),
    ("Gauss", -0.5,  make_pot_Gauss),
    # ("RK", -0.5,  make_pot_RK),
    # ("X",  -0.5,  make_pot_X),
    # ("P",  -0.5,  make_pot_P),
]
...
class H_1e_D_Z_Lcr_run(FlowTest):
    ...
    def load_test_1(self):
        ...
        # Z=2, Gauss, L=-0.5: [-3.280258  1.258711  3.322435]
        # 1 & $1s$ & $-0.946982$ & $0.526509$ \\
        # 1 & $2s$ & $1.322503$ & $1.661252$ \\
        # 1 & $3s$ & $2.354365$ & $2.177182$ \\
        # \midrule
        # 2 & $1s$ & $-3.280258$ & $0.526509$ \\
        # 2 & $2s$ & $1.258711$ & $1.661251$ \\
        # 2 & $3s$ & $3.322435$ & $2.177182$ \\

        orth1N = 40; lambda_basis = 2 * cfg.atom_z; nx1 = 2001;  x1_max = 5.;   x1_min = -12.
        # Z=2, Gauss, L=-0.5: [-3.280258  1.258711  3.322435]
        # 1 & $1s$ & $-0.946982$ & $0.526509$ \\
        # 1 & $2s$ & $1.322503$ & $1.661251$ \\
        # 1 & $3s$ & $2.354365$ & $2.177182$ \\
        # \midrule
        # 2 & $1s$ & $-3.280258$ & $0.526509$ \\
        # 2 & $2s$ & $1.258711$ & $1.661251$ \\
    ...
    def run_test_1(self, potFunc):
        ...
        return eigEngs.arr, eig_dimless
...
def main():
    ...
    for Z in [1.0, 2.0]:
        ...
        for name, L, make_pot in pots:
            ...
            runner.load_test_1()
            potFunc = make_pot(Z)
            engs, eig_dimless = runner.run_test_1(potFunc)
            results[Z][name] = engs[:3]
            results[Z][name + '_dl'] = eig_dimless[:3]
            print(
    ...
    for Z in [1.0, 2.0]:
        for i, n in enumerate([1, 2, 3]):
            gauss = results[Z]["Gauss"][i]
            gauss_dl = results[Z]["Gauss_dl"][i]
            # e3d = results[Z]["3D"][i]
            # erk = results[Z]["RK"][i]
            # ex = results[Z]["X"][i]
            # ep = results[Z]["P"][i]
